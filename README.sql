...
create or replace function chronomodel_countries_update() returns trigger as $$
  ...
  begin
    ...
    perform 1 from temporal.countries where id = old.id for update;

    _now := timezone('UTC', now());
    _hid := null;

    select hid into _hid from history.countries where id = old.id and lower(validity) >= _now order by lower(validity) desc, hid desc limit 1;
