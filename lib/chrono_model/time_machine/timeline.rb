  ...
  module TimeMachine
    ...
    module Timeline
      ...
      def timeline(record = nil, options = {})
        ...
        if assocs.present?
          assocs.each do |ass|
            # join first, then use  wheres
            ## is it an has many or a belongs to?
            if ass.belongs_to?
              relation = relation.joins("LEFT JOIN #{ass.table_name} ON #{ass.table_name}.id = #{table_name}.#{ass.foreign_key}")
            else
              relation = relation.joins("LEFT JOIN #{ass.table_name} ON #{ass.table_name}.#{ass.foreign_key} = #{table_name}.id")
            end
          end
        end
